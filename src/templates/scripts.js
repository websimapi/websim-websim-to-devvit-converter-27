...
files.forEach(fileObj => {
    ...
    if (f.endsWith('.html') || f.endsWith('.js')) {
        ...
        if (importMatch || remoteMatch) {
            console.error(\`❌ EXTERNAL RESOURCE DETECTED in \${f}\`);
            console.error(\`   Devvit Web Apps MUST NOT access external URLs from the client (CSP violation).\`);
            const match = importMatch || remoteMatch;
            if (match) console.error(\`   Target: \${match[1]}\`);
            console.error(\`   Solution: Move this logic to src/server/index.ts (server-side fetch) or download the asset locally.\`);
            issues++;
            // Critical failure for Devvit Web
            process.exitCode = 1;
        }
...
if (issues > 0) {
    console.log(\`\\n⚠️  Found \${issues} issues. See above.\`);
    if (process.exitCode === 1) {
        console.error('❌ Validation Failed: The client bundle contains external URLs which are strictly forbidden.');
        process.exit(1);
    }
} else {
